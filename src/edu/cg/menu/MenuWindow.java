...
public class MenuWindow extends JFrame implements Logger {
	...
	private final ScaleSelector scaleSelector;
	...
	public void removeObjectFromImage(boolean[][] srcMask) {
		...
		int W;
		int Max_inRow;
		int Seams_Change_NUM;
		SeamsCarver sc;
		boolean[][] new_mask;
		RGBWeights rgbWeights;

		outWidth = srcMask[0].length;
		W = (srcMask[0].length / 3) -1;
		Max_inRow = 0;
		for (int i = 0; i < srcMask.length; i++) {
			...
			if(Seams_inRow_NUM > Max_inRow){
				Max_inRow = Seams_inRow_NUM;
			}
		}
		Seams_Change_NUM = Math.min(W,Max_inRow);

		rgbWeights = colorMixer.getRGBWeights();
		scaleSelector.setWidth(scaleSelector.width() - Seams_Change_NUM);
		sc = new SeamsCarver(this, duplicateImage(), outWidth-Seams_Change_NUM, rgbWeights, srcMask);
		img = sc.resize();
		new_mask = sc.getMaskAfterSeamCarving();
